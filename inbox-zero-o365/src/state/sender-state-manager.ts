...
export async function writeSenderState(filePath: string, state: SenderStateFile): Promise<void> {
  const parsed = SenderStateFileSchema.safeParse(state);
  if (!parsed.success) {
    throw new Error(`Invalid sender state: ${parsed.error.message}`);
  }
...
export function mergeSenderState(fresh: SenderStateEntry[], existing: SenderStateEntry[]): SenderStateEntry[] {
  ...
  return fresh.map((freshEntry) => {
    ...
    // Preserve persisted optional state, then overlay freshly recomputed stats.
    const merged: SenderStateEntry = { ...prev, ...freshEntry };
