  ...
  for (const email of emails) {
    ...
    if (acc === undefined) {
      acc = {
        ...
        latestNameDate: email.sender.name === "" ? new Date(0) : email.dateReceived,
        emailCount: 0,
        unreadCount: 0,
        starredCount: 0,
        importantCount: 0,
    ...
    // display name: prefer the most recent non-empty name.
    // If the current name is empty, any non-empty name wins regardless of date.
    if (email.sender.name !== "" && (acc.senderName === "" || email.dateReceived >= acc.latestNameDate)) {
