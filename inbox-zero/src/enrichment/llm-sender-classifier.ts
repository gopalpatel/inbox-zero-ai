  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    while (retries < 2) {
      try {
        ...
        if (parsed.length > 0) {
          const parsedByEmail = new Map(parsed.map((entry) => [entry.email.toLowerCase(), entry] as const));

          for (const entry of parsed) {
            results.set(entry.email.toLowerCase(), { senderType: entry.senderType, confidence: entry.confidence });
          }
